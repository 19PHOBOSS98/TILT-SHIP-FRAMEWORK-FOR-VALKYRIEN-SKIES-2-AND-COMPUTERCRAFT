...
function adjustOrbitRadiusPosition(target_g_pos,orbit_target_pos,radius)--flightUtilities
	local radius_vector = (target_g_pos:sub(orbit_target_pos)):normalize()*radius
	return orbit_target_pos:add(radius_vector)
end

function getLocalPositionError(trg_g_pos,current_g_pos,current_rot)--flightUtilities
...
function getQuaternionRotationError(target_rot,current_rot)--flightUtilities
