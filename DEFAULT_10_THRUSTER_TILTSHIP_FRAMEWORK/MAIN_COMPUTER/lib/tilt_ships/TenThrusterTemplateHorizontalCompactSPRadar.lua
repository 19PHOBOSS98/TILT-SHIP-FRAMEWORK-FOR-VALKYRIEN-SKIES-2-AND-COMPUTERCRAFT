...
local TenThrusterTemplateHorizontalCompactSPRadar = RemoteControlDrone:subclass()

function TenThrusterTemplateHorizontalCompactSPRadar:getOffsetDefaultShipOrientation(default_ship_orientation)
	return quaternion.fromRotation(default_ship_orientation:localPositiveZ(), 45)*default_ship_orientation
end

function TenThrusterTemplateHorizontalCompactSPRadar:initPeripherals()
	self.sp_radar = peripheral.find("sp_radar")
	self.goggles = peripheral.find("goggle_link_port")
	self.modem = peripheral.find("modem", function(name, object) return object.isWireless() end)
end

function TenThrusterTemplateHorizontalCompactSPRadar:initVariables()
	local sp_radar = self.sp_radar
	local local_ship_readings = sp_radar.scan(1)
	
	self.ship_global_velocity = vector.new(0,0,0)
	self.run_firmware = true
	
	self.ship_rotation = local_ship_readings.rotation
	...
	self.ship_global_position = local_ship_readings.pos
...
function TenThrusterTemplateHorizontalCompactSPRadar:init(instance_configs)
	...
	TenThrusterTemplateHorizontalCompactSPRadar.superClass.init(self,configs)
end

function TenThrusterTemplateHorizontalCompactSPRadar:composeComponentMessage(linear,angular)
...
TenThrusterTemplateHorizontalCompactSPRadar.RSIBow = peripheral.wrap("front")
TenThrusterTemplateHorizontalCompactSPRadar.RSIStern = peripheral.wrap("back")

function TenThrusterTemplateHorizontalCompactSPRadar:powerThrusters(group,component_values)
	if (group == "BOW") then
...
function TenThrusterTemplateHorizontalCompactSPRadar:communicateWithComponent(component_control_msg)	
	self:powerThrusters("BOW",component_control_msg.BOW)
	self:powerThrusters("STERN",component_control_msg.STERN)
end

function TenThrusterTemplateHorizontalCompactSPRadar:resetRedstone()
...
return TenThrusterTemplateHorizontalCompactSPRadar